...
while fold_count < args.fs_folds:
    tr_idxs, cv_idxs = train_test_split(np.arange(y.size), test_size=0.3, stratify=y)
    if print_header:
    ...
    fs_data['y_scores_all'].extend(y_score.tolist())
    fs_data['y_tests_all'].extend(y[cv_idxs].tolist())
...
print("ROC AUC:", roc_auc_score(fs_data['y_tests_all'], fs_data['y_scores_all']))
