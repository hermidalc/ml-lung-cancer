...
#    - ... values to pass to factorization method (typically, alpha value if factorization by stICA)
#
#  Outputs:
#
...
#    Renard E., Branders S. and Absil P.-A.: Independent Component Analysis to Remove Batch Effects from Merged Microarray Datasets (WABI2016)
   ...
   if (!is.null(ref2)){
      ...
      for (i in 1:length(refType2)){
         ...
         idx_2 = c(idx_2,which(factR2_2$allR2[,i]>t2[i]))
      }


      idx2keep =intersect(idx,idx_2)
      print(paste("Keeping",length(idx2keep), "cmpts with R2(ref2) higher than t2"))
   ...
   print(paste("Removing",length(idx),"components with R2(ref) higher than",t))
