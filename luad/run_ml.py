...
def pipeline_1(eset_gex, fs_meth, tr_meth, color):
    ...
    # end for
    y_tests, y_scores = [], []
    for tr_split_data in tr_data:
        y_tests.extend(tr_split_data['y_tests'])
        y_scores.extend(tr_split_data['y_scores'])
...
def process_fs_data(fs_data):
    ...
    for idx in range(len(fs_feature_idxs)):
        ...
        #     coef_mx[idx]
        # )
    roc_auc_mx = np.zeros((len(fs_feature_idxs), args.fs_splits), dtype=float)
    for split_idx in range(len(fs_data['split_data'])):
        split_data = fs_data['split_data'][split_idx]
        for idx in range(len(split_data['feature_idxs'])):
    ...
    for idx in range(len(fs_feature_idxs)):
        fs_data['feature_mean_roc_aucs'].append(
            statistics.mean(roc_auc_mx[idx])
        )
        # print(
        #     fs_feature_names[idx], "\t",
    ...
    fs_rank_data = fs_rank_data[:fs_top_cutoff]
    fs_feature_idxs = np.array([x for _, x, _ in fs_rank_data], dtype=int)
    fs_feature_names = np.array([x for _, _, x in fs_rank_data], dtype=str)
